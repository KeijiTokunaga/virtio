...
## 初学者が誤解しやすいポイント（virtioの境界と前提）

virtioを学ぶ際、アーキテクチャの境界を見誤ると設計やデバッグで混乱しやすくなります。以下の前提を理解しておくことが重要です。

1. **「virtioはソケット通信」ではない**:
   virtioの本質は **共有メモリ（DMAバッファ＋リングバッファ）** によるデバイスI/Oです。TCP/IPやネットワークスタックは通りません。このリポジトリのデモでQEMUがUnixドメインソケットを使っているのは、QEMU内部のvirtioバックエンドとホスト上のC++プログラム（`src/host.cpp`）を中継するためのQEMU独自の構成（chardev）であり、virtio仕様自体の要件ではありません。
2. **「アプリが直接virtqueueを操作する」のではない**:
   virtqueueやDescriptor Table、リングバッファ、PCI割り込みを操作するのは **OSカーネル内のvirtioドライバー（Frontend）** です。アプリケーション（`src/guest.cpp`）は、通常のファイルと同じように `/dev/virtio-ports/...` に対して標準の `open / read / write` システムコールを呼ぶだけで通信できます。
3. **「write()の成功＝ホスト処理完了」ではない**:
   ゲストの `write()` が成功しても、それは「ドライバーの送信キューにデータが渡された」ことしか意味しません。ホスト側の処理完了や返信の到着は、受信キューへの返却とゲストへの通知を待つ非同期パイプラインです。
4. **「ホストとゲストでメモリアドレス（ポインタ）を直接共有できる」わけではない**:
   ゲストの仮想アドレス（GVA）とホストの仮想アドレス（HVA）は完全に別空間です。ディスクリプタに積まれるのは **ゲスト物理アドレス（GPA）** であり、ホスト側（QEMUやQNX vdev）がハイパーバイザーのStage-2マッピングを通して自空間のアドレスへ変換して初めてアクセスできます。
5. **「キャラクタデバイスでもパケット（メッセージ）境界は保持されない」**:
   `/dev/virtio-ports/...` はバイトストリームです。1回の `write()` で送ったデータが複数回の `read()` に分かれたり、連続送信が1回にまとまることがあります。そのため `src/transport.hpp` のように、アプリケーション層で改行区切りなどのフレーミング処理が必要です。
6. **「キック（Doorbell / 通知）はタダではない」**:
   ゲストからホストへのキックは、PCI/MMIOレジスタ書き込みに伴う **VM-Exit（CPU仮想化トラップ）** を引き起こします。通常のメモリアクセスに比べ数千倍重いため、複数バッファをまとめて通知するバッチ処理や、通知抑制フラグ（`VIRTIO_F_EVENT_IDX` 等）が不可欠です。
