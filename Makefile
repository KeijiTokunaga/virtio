...
# 自動デモ用Linuxバイナリはsrc/demo.cppが別途クロスコンパイルする。
CXX ?= c++
CXXFLAGS ?= -std=c++17 -O2 -Wall -Wextra -Wpedantic

.PHONY: all test demo clean
all: build/host build/guest build/demo
...
build/demo: src/demo.cpp src/transport.hpp | build
	$(CXX) $(CXXFLAGS) -Isrc -pthread src/demo.cpp -o $@
